...
async function $3b3adf5e97fa5fd8$export$2e2bcd8739ae039({ config: config , code: code  }) {
    ...
    const content = await (0, $hgUW1$nodefspromises).readFile((0, $hgUW1$nodepath).join($hgUW1$cwd(), "node_modules/mermaid/dist/mermaid.min.js"), "utf8");
    await page.addScriptTag({
        content: content
    });
    const result = await page.evaluate(([diagramCode])=>{
        window.mermaid.initialize({});
        try {
            /* Render the mermaid diagram */ const svgCode = window.mermaid.mermaidAPI.render("diagram", diagramCode);
            return {
                status: "success",
                svgCode: svgCode
            };
